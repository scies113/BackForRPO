...
### [2026-05-05] — Полная интеграция ML-модели и Docker Compose (v4.0.0)

**Выполнено:**
- ✅ **Docker Compose**: Полное объединение PostgreSQL, Go-бэкенда и Python ML-модели в единый стек.
- ✅ **ML-Сервис**: FastAPI сервис на порту 8000 с поддержкой эндпоинтов `/predict`, `/health`, `/teams`.
- ✅ **Go-Бэкенд**: Интеграция с ML через `ML_PREDICT_URL`, динамическое обращение во внутреннюю сеть Docker (`http://ml:8000`).
- ✅ **Оптимизация сборки**: Добавлены `.dockerignore` файлы для значительного ускорения сборки образов.
- ✅ **Документация ML**: Создан файл `TEAMS.md` с точными названиями команд для корректной работы XGBoost.
- ✅ **Исправление фронтенда**: Поля `home_win_prob` и др. в `predict_handler.go` синхронизированы с ожиданиями фронтенда.

**Результат:** 100% готовности, продакшен-сборка.
